...
const STORAGE_KEY = "py-visualizer-state-old";
...
`);
  ...
  const teardownViz = useCallback(() => {
    ...
    const host = typeof document !== "undefined" ? document.getElementById("opt-viz-old") : null;
  ...
  const redraw = useCallback(() => {
    ...
    const host = document.getElementById("opt-viz-old");
...
`);
      ...
      const viz = new window.ExecutionVisualizer("opt-viz-old", { code: source, trace }, {
  ...
  return (
    <div
      className={`pyold-root ${showContent ? "show" : ""} ${expanded ? "expanded" : ""} ${dragging ? "dragging" : ""}`}
      style={rootStyle}
    >
      <header className="py-header">
        <div className="py-header-inner">
          ...
          <nav className="py-nav">
            <a href="/python" className="nav-btn">
